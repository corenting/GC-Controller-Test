...
int main(int argc, char **argv) {
    ...
    console_init(xfb, CONSOLE_START_POS, CONSOLE_START_POS, rMode->fbWidth, rMode->xfbHeight,
    ...
    while (1) {
        ...
        printf("	Stick value (X,Y)   : %03d,%03d %s\n", PAD_StickX(activePad), PAD_StickY(activePad),
               GetPadDirection(PAD_StickX(activePad), PAD_StickY(activePad)));
        printf("	C-stick value (X,Y) : %03d,%03d %s\n", PAD_SubStickX(activePad), PAD_SubStickY(activePad),
               GetPadDirection(PAD_SubStickX(activePad), PAD_SubStickY(activePad)));


        //Special actions
        if (keysHeld[activePad] & PAD_BUTTON_A && keysHeldOld[activePad] & PAD_BUTTON_B) {
            PAD_ControlMotor(activePad, 1);
        }
#ifdef __WII__
        if (keysHeld[activePad] & PAD_TRIGGER_L && keysHeldOld[activePad] & PAD_TRIGGER_R) {
            exit(0);
        }
#endif
